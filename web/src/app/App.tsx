...
function ProjectsPage({ isAdmin, notify }: { isAdmin: boolean; notify: (text: string, error?: boolean) => void }) {
  ...
  return <div className="page"><PageHeader title={t("projects")} action={isAdmin ? <Button variant="primary" icon={<Plus />} label={`${t("new")} ${t("project")}`} onClick={() => setEditing("new")} /> : undefined} />
    {loading ? <PageSkeleton /> : <><ListToolbar query={query} onQuery={setQuery} />
...
function ProjectDialog({ item, onClose, onSave }: { item: Project | null; onClose: () => void; onSave: (name: string) => Promise<void> }) {
  ...
  return <Modal title={`${item ? t("edit") : t("new")} ${t("project")}`} onClose={onClose}><form className="form-stack" onSubmit={submit}>{!item && <Banner status="info" title={t("newProjectHint")} />}<Field label={t("projectName")} value={name} onChange={setName} autoFocus /><DialogActions onClose={onClose} busy={busy} disabled={!name.trim()} /></form></Modal>
}
...
function UsersPage({ currentUser, notify }: { currentUser: User; notify: (text: string, error?: boolean) => void }) {
  const { t } = useI18n(); const [items, setItems] = useState<User[]>([]); const [loading, setLoading] = useState(true); const [editing, setEditing] = useState<User | "new" | null>(null); const [passwordUser, setPasswordUser] = useState<User | null>(null)
