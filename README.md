...
Set `LIGHTBWS_IMAGE=ghcr.io/ca-x/lightbws:0.2.5` in `.env` when a deployment must remain pinned to a specific release. Running `docker compose down` keeps the data volume; `docker compose down -v` permanently deletes it.
...
git tag -a v0.2.5 -m "LightBWS v0.2.5"
